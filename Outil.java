...
/**
 * Cette contient l'ensemble des méthode nécessaire pour assurer le bon
 * fonctionnement de l'algorithme de Chang & Robert
 */
public class Outil {
	...
	public static void periodiqueVerification(ORB orb, Orb_Run orb_run,
			int uid, String successeur, String successeurPanne,
			processusHolder ref) {
		SubTimer sbTimer = new SubTimer(orb, orb_run, uid, successeur,
				successeurPanne, ref);
	...
	public static void verifierSuccesseur(ORB orb, Orb_Run orb_run, int uid,
			String successeur, String successeurPanne, processusHolder ref) {
		boolean exists = false;
	...
	public static void election(ORB orb, Orb_Run orb_run, int uid) {
		...
		while (!successeur.equals("" + uid)) {
			...
			if (!fichier.exists()) {
				System.out.println("Le processus p" + successeur
						+ " est inexistant!!");
				break;
			}
		...
		if (found) {
			...
			successeur = p.successeur();
			// On se positionne sur l'element qui suit celui qui a déclenché
			// l'election afin d'invoquer
			// la méthode recevoir
			p = Outil.lookupRef(successeur, orb);

			while (!fin_algo) {

				System.out.println("Le processus p" + p.uid()
						+ " recoit l'uid " + max.value);
				p.recevoir(max);
				successeur = p.successeur();
				p = Outil.lookupRef(successeur, orb);
				if (p.uid() == max.value) {
	...
	public static void preparationObjetCoteServeur(int uid, String successeur,
			String successeurPanne, ORB orb, POA poa) {
		try {
			poa = POAHelper.narrow(orb.resolve_initial_references("RootPOA"));
			poa.the_POAManager().activate();
			processusImpl machine = new processusImpl(uid, successeur,
					successeurPanne);
			...
			try {
				...
				System.err.println("Impossible d'écrire la reference dans p"
						+ uid + ".ref");
				System.exit(1);
			}
		} catch (Exception e) {
	...
	public static void reconstituerAnneau(int id, String successeur, ORB orb,
			processusHolder ref) {
